...
# 绘制第一个条形图，颜色较深
ax1.bar(x, totalActiveTimePerDay, width, alpha=1, color='white', edgecolor=edgecolors, label='TS 时长')

# 绘制第二个条形图，颜色较浅
ax1.bar(x, userActiveTimePerDay, width, color=AppColor.C_11_3[1], edgecolor=edgecolors, label='IS 时长')
ax1.text(max(x) - 11, max(totalActiveTimePerDay) - 40, f"NILR: {non_interactive_rate}%", ha='right', va='top')
ax1.set_xlabel('用户')
ax1.set_ylabel('时长 (min)')
ax1.legend()
