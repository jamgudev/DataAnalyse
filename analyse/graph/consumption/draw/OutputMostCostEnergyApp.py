from analyse.graph.GrapgNameSapce import GRAPH_app_page_consumption, GRAPH_most_cost_energy_app
from analyse.util.FilePathDefinition import TEST_OUTPUT_FILE
from util import ExcelUtil


def output_most_cost_energy_app():
    # 读取Excel数据
    ...
    grouped_data = data.groupby([data.iloc[:, user_idx_col], data.iloc[:, user_name_col], data.iloc[:, phone_brand_col],
                     data.iloc[:, app_category_col], data.iloc[:, app_name_col]])[[stay_duration_col,
                                                   stay_duration_rate_col, app_energy_cost_rate_col]].sum()
    ...
    filtered_df = sorted_df[sorted_df.iloc[:, 1] >= 0.00]
    ...
    output.to_excel(TEST_OUTPUT_FILE + "/" + GRAPH_most_cost_energy_app, index=False)
