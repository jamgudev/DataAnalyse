...
from analyse.graph.GrapgNameSapce import GRAPH_app_page_consumption, GRAPH_most_cost_energy_app_category
...
grouped_data = data.groupby([data.iloc[:, user_idx_col], data.iloc[:, user_name_col], data.iloc[:, phone_brand_col],
                 data.iloc[:, app_category_col]])[[stay_duration_col,
...
output.to_excel(TEST_OUTPUT_FILE + "/" + GRAPH_most_cost_energy_app_category, index=False)
