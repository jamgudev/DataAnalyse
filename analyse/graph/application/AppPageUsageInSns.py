...
from analyse.graph.GrapgNameSapce import AD_APP_PAGE_NAME, AD_APP_DURATION, \
    AD_APP_PKG_NAME, GRAPH_app_page_usage_in_sns
    ...
    if allUserName:
        ...
        with alive_bar(len(allUserName), ctrl_c=True, force_tty=True, title=f'分析进度') as bar:
                    ...
                    for idx, data in enumerate(dataOfEveryDay.values()):
                        try:
                            ...
                            for pkg_idx, pkg_name in enumerate(data[0]):
                                if AppCategory.get_app_category(dirName, pkg_name, appCategoryDict) != "sns":
                                    continue
            ...
            ExcelUtil.write_to_excel(allUserData, dirName,
                                     GRAPH_app_page_usage_in_sns)


app_page_usage_in_sns()
